...
print("Solving...")
sol = solve(prob,saveat = 0.5,progress = true)
print("\n Done!")

g_sol = reshape(sol[3 * RX + 1:4 * RX,:], R1,R2, X, size(sol, 2)) #conductance
V_sol = reshape(sol[4 * RX + 1:5 * RX,:], R1,R2, X, size(sol, 2)) #voltage

npzwrite("/home/james/PhD_Work/Python_Code/Axo_Dendritic_Paper/PythonGraphingScripts/DataGraphing/Conductance_3D.npy",g_sol)
npzwrite("/home/james/PhD_Work/Python_Code/Axo_Dendritic_Paper/PythonGraphingScripts/DataGraphing/Voltage_3D.npy",V_sol)
