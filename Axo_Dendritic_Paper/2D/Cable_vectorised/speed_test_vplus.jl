...
const pos = 0.04 #position on cable (multiples of dx)
const alfa = 1
const h = 0.15
#GRID DISCRETISATION
#R (r) - soma space
...
vplus_vec = LinRange(5,120,60)

c_numeric_vector_k = zeros(size(vplus_vec,1))

for i = 1:size(vplus_vec,1)
    global u0
    global vplus_vec
    global V_plus = vplus_vec[i]
...
print("Solving...")
condition(u,t,integrator) = reshape(u[3*RX+1:4*RX],R,X)[Int(85/dr),d]>0.05
affect!(integrator) = terminate!(integrator)
cb = DiscreteCallback(condition,affect!)
sol = solve(prob,RK4(),saveat = dt,progress = true,callback = cb)
print("\n Done! \n")
g_sol =  reshape(sol[3 * RX + 1:4 * RX,:], R, X, size(sol, 2))
c_numeric_vector_k[i] = trackSpeed(g_sol[:,d,:],Int(60/dr),Int(70/dr),dt,dr,h,size(g_sol,3),0.05)
print("c = ",c_numeric_vector_k[i],"\n")
end

npzwrite("/tmp/c_numeric_vplus.npy",c_numeric_vector_k)
